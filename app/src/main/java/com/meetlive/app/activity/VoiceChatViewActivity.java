...
public class VoiceChatViewActivity extends AppCompatActivity implements ApiResponseInterface {
    ...
    private static final String LOG_TAG = com.meetlive.app.activity.VoiceChatViewActivity.class.getSimpleName();
    ...
    private String token = "", UID = "", unique_id = "", profileImage = "", userpoints, receiveraudiocallRate;
    long AUTO_END_TIME;
    ...
    private void getInData() {
        ...
        if (b != null) {
            ...
            profileImage = b.getString("profile_image");

            AUTO_END_TIME = getIntent().getIntExtra("AUTO_END_TIME", 2000);

            Glide.with(getApplicationContext())
                    .load(profileImage)
                    .into(((ImageView) findViewById(R.id.img_profileview)));
           ...
           /* Log.e("token",token);
            ...
            if (new SessionManager(getApplicationContext()).getGender().equals("female")) {
                userpoints = b.getString("userpoints");
                receiveraudiocallRate = b.getString("receiveraudiocallRate");

                int talkTime = Integer.parseInt(userpoints) / Integer.parseInt(receiveraudiocallRate) * 1000 * 60;

                //  int talkTime2 = userData.getCall_rate() * 1000 * 60;
                // Minus 2 sec to prevent balance goes into minus
                AUTO_END_TIME = talkTime - 2000;
            }
    ...
    private void joinChannel() {
        ...
        mRtcEngine.joinChannel(accessToken, "zeeplive" + unique_id, "Extra Optional Data", 0); // if you do not specify the uid, we will generate the uid for you

        if (new SessionManager(getApplicationContext()).getGender().equals("female")) {
            talkTimeHandler.postDelayed(() -> {
                        endCall();
                        Toast.makeText(VoiceChatViewActivity.this, "Out of Balance", Toast.LENGTH_LONG).show();
                    }
                    , AUTO_END_TIME);


            new ApiManager(this, this).VoiceCallData(new CallRecordBodyAudio(UID, unique_id, "audio",
                    new CallRecordBodyAudio.Duration(String.valueOf(System.currentTimeMillis()), "")));
